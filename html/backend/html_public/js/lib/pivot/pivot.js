  ...
  callWithJQuery(function($) {
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              push: function(record) {
                  ...
                  return this.sum += parseFloat(record[attr].replace('.', '').replace(',', '.'));
                }
              },
              value: function() {
                return this.sum;
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              push: function(record) {
                var x;
                if (!isNaN(parseFloat(record[attr]))) {
                      record[attr] = record[attr].replace('.', '').replace(',', '.');
                  }
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              push: function(record) {
                ...
                x = parseFloat(record[attr].replace('.', '').replace(',', '.'));
                this.n += 1.0;
                if (this.n === 1.0) {
                  return this.m = x;
                } else {
            ...
            return {
              ...
              push: function(record) {
                if (!isNaN(parseFloat(record[num]))) {
                  this.sumNum += parseFloat(record[num].replace('.', '').replace(',', '.'));
                }
                if (!isNaN(parseFloat(record[denom]))) {
                  return this.sumDenom += parseFloat(record[denom].replace('.', '').replace(',', '.'));
                }
              },
              value: function() {
    ...
    aggregators = (function(tpl) {
      return {
        ...
        "Suma de sumas": tpl.sumOverSum(usFmt),
        "Cota 80% superior": tpl.sumOverSumBound80(true, usFmt),
        "Cota 80% inferior": tpl.sumOverSumBound80(false, usFmt),
        "Proporción del total (suma)": tpl.fractionOf(tpl.sum(), "total", usFmtPct),
        "Proporción de la fila (suma)": tpl.fractionOf(tpl.sum(), "row", usFmtPct),
        "Proporción de la columna (suma)": tpl.fractionOf(tpl.sum(), "col", usFmtPct),
        "Proporción del total (cuenta)": tpl.fractionOf(tpl.count(), "total", usFmtPct),
        "Proporción de la fila (cuenta)": tpl.fractionOf(tpl.count(), "row", usFmtPct),
        "Proporción de la columna (cuenta)": tpl.fractionOf(tpl.count(), "col", usFmtPct)
      };
    })(aggregatorTemplates);
    renderers = {
      "Pivot Generado": function(data, opts) {
    ...
    locales = {
      en: {
        ...
        renderers: renderers,
        localeStrings: {
          renderError: "O_o Algo pasó al renderizar la tabla.",
          computeError: "O_o Algo pasó al computar los resultados.",
          uiRenderError: "O_o Algo pasó al renderizar el UI.",
          selectAll: "Marcar Todo",
          selectNone: "Desmarcar Todo",
          tooMany: "(hay mucho para listar)",
    ...
    $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
      ...
      try {
          ...
          if (values.length > opts.menuLimit) {
            ...
            if (values.length > 5) {
              ...
              $("<input>", {
                ...
                return valueList.find('.pvtCheckContainer p label label strong.value').each(function() {
                  if (accept($(this).text())) {
              ...
              controls.append($("<br>"));
              $("<button class='BotonMinimalistaPequeno'>", {
                type: "button"
            ...
            for (n = 0, len2 = ref1.length; n < len2; n++) {
			  ...
			  valor_random = Math.floor((Math.random() * 999999999) + 1);
			  
              $("<input style='opacity:0; position:absolute; display: block;' id='pivot_" + valor_random + "'>").attr("type", "checkbox").addClass('pvtFilter').attr("checked", !filterItemExcluded).data("filter", [attr, value]).appendTo(filterItem).bind("change", function() {
                return $(this).toggleClass("changed");
              });
			  
			  filterItem2 = $("<label for='pivot_" + valor_random + "' class=''>");
			  filterItem2.append($("<span>").text(""));
			  filterItem2.append($("<strong>").addClass("value").text(value));
			  filterItem2.append($("<strong>").addClass("count").text("(" + valueCount + ")"));
			  
			  filterItem.append(filterItem2);
        ...
        pivotTable = $("<td id=\"tabla_resultado_pivot\">").attr("valign", "top").addClass('pvtRendererArea').appendTo(tr2);
        if (opts.unusedAttrsVertical === true || unusedAttrsVerticalAutoOverride) {
