...
class Dashboard_controller extends MY_Controller {
    ...
    private function ArmarValoresFiltro($sel_departamento, $sel_agencia, $sel_oficial, $campoFiltroFechaDesde, $campoFiltroFechaHasta, $campoFiltroFechaDCDesde, $campoFiltroFechaDCHasta)
    {
        $nombres_filtro = '';
        ...
        if($campoFiltroFechaDCDesde != '' || $campoFiltroFechaDCHasta != '')
        {
            $nombres_filtro .= '<b>*Fecha Desembolso COBIS: </b> ';
            
            if($this->mfunciones_generales->VerificaFechaD_M_Y($campoFiltroFechaDCDesde))
            {
                $nombres_filtro .= ' <i>Desde el:</i> ' . $campoFiltroFechaDCDesde;
            }

            if($this->mfunciones_generales->VerificaFechaD_M_Y($campoFiltroFechaDCHasta))
            {
                $nombres_filtro .= ' <i>Hasta el:</i> ' . $campoFiltroFechaDCHasta;
            }
    ...
    public function ReporteFunnel()  {
        ...
        $campoFiltroFechaDCDesde = $this->input->post('campoFiltroFechaDCDesde', TRUE);
        $campoFiltroFechaDCHasta = $this->input->post('campoFiltroFechaDCHasta', TRUE);
        
        $filtro = $this->mfunciones_dashboard->ArmarFiltro($sel_departamento, $sel_agencia, $sel_oficial, $campoFiltroFechaDesde, $campoFiltroFechaHasta, 0, '', '');
        
        $resultado = $this->mfunciones_dashboard->GenerarFunnel($filtro, $campoFiltroFechaDCDesde, $campoFiltroFechaDCHasta);
        
        $data['funnel'] = $resultado->funnel;
        $data['chartFunnel'] = $resultado->chartFunnel;
        
        js_invocacion_javascript('$("html,body").animate({scrollTop: $("#FormularioOpcionesReporte").show().offset().top - 80}, 600);');
    ...
    public function ReporteFunnel_Tabla()  {
        ...
        $etapa_aux = $etapa = str_replace('&', '', $this->input->post('etapa', TRUE));
        
            // Condición auxiliar, si se está mandando el estado desembolsado con rango de fechas, es el cód "9024" se mantiene las fechas capturadas y se debe cambiar por 24.
            if((int)$etapa_aux == 9024)
            {
                $etapa = 24;
            }
            else
            {
                // Si no es el caso auxiliar "9024" entonces se limpia las fechas de desembolso COBIS
                $campoFiltroFechaDCDesde = '';
                $campoFiltroFechaDCHasta = '';
            }
        
        $ValoresFiltro = $this->ArmarValoresFiltro($sel_departamento, $sel_agencia, $sel_oficial, $campoFiltroFechaDesde, $campoFiltroFechaHasta, $campoFiltroFechaDCDesde, $campoFiltroFechaDCHasta);
        ...
        switch ((int)$etapa) {
            ...
            default:
                ...
                {
                    $etapa_nombre = $arrEtapas[0]['etapa_nombre'] . ((int)$etapa_aux==9024 ? ' (EN EL RANGO SELECCIONADO)' : '');
                }
        ...
        $filtro = 'AND etapa_id IN(' . $etapa . ')' . $this->mfunciones_dashboard->ArmarFiltro($sel_departamento, $sel_agencia, $sel_oficial, $campoFiltroFechaDesde, $campoFiltroFechaHasta, 0, $campoFiltroFechaDCDesde, $campoFiltroFechaDCHasta);
