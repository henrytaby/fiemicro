...
class Key extends REST_Controller
{
	...
	public function index_put()
    {
		...
		else
		{
			$this->response(array(
                                    'error' => true,
                                    'errorMessage' => 'No se pudo guardar la llave.',
                                    'errorCode' => 111,
                                    'status' => 0, 
                                    'error_detail' => 'Could not save the key.'), 500); // 500 = Internal Server Error
		}
    }
	...
	public function index_delete()
    {
		...
		if ( ! self::_key_exists($key))
		{
			...
			$this->response(array(
                            ...
                            'errorCode' => 112,
                            'status' => 0,
                            'error_detail' => 'Invalid API Key.'), 400);
		}
		...
		$this->response(array(
                        ...
                        'errorCode' => 0,
                        'status' => 1, 
                        'success' => 'API Key was deleted.'), 200);
    }
	...
	public function level_post()
    {
		...
		if (self::_update_key($key, array('level' => $new_level)))
		{
			$this->response(array(
                            ...
                            'errorCode' => 0,
                            'status' => 1, 
                            'success' => 'API Key was updated.'), 200); // 200 = OK
		}
		...
		else
		{
			$this->response(array(
                            'error' => true,
                            'errorMessage' => 'No se puede actualizar el nivel de la Key.',
                            'errorCode' => 113,
                            'status' => 0, 
                            'error_detail' => 'Could not update the key level.'), 500); // 500 = Internal Server Error
		}
    }
	...
	public function suspend_post()
    {
		...
		if (self::_update_key($key, array('level' => 0)))
		{
			$this->response(array(
                            ...
                            'errorCode' => 0,
                            'status' => 1, 
                            'success' => 'Key was suspended.'), 200); // 200 = OK
		}
		...
		else
		{
			$this->response(array(
                            'error' => true,
                            'errorMessage' => 'No se pudo suspender el usuario.',
                            'errorCode' => 114,
                            'status' => 0, 
                            'error_detail' => 'Could not suspend the user.'), 500); // 500 = Internal Server Error
		}
    }
