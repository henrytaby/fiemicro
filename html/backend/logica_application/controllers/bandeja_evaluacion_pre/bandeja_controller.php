...
class Bandeja_controller extends MY_Controller {
	...
	protected $codigo_menu_acceso = 19;
        
        // Código de la Etapa Propio de la Bandeja
        protected $codigo_etapa = 12;
    ...
    public function Bandeja_Ver() {
        ...
        $filtro = 'p.prospecto_etapa=' . $this->codigo_etapa . ' AND p.prospecto_estado_actual=1 AND p.prospecto_rechazado=0 AND p.prospecto_consolidado=0 AND p.prospecto_observado_app=0';
        ...
        $_SESSION['direccion_bandeja_actual'] = 'Bandeja/Preafiliacion/Ver';
        ...
        $this->load->view('bandeja_evaluacion_pre/view_bandeja_ver', $data);
    }
    
    public function Antecedentes_Form() {
        $this->lang->load('general', 'castellano');
        $this->load->model('mfunciones_generales');
        $this->load->model('mfunciones_logica');
        ...
        else
        {
            ...
            $this->load->view('bandeja_evaluacion_pre/view_antecedentes_form', $data);
        }
    }
    
    public function Antecedentes_Guardar() {
        ...
        {        
            ...
            $pep = $this->input->post('pep', TRUE);
            $match = $this->input->post('match', TRUE);
            $infocred = $this->input->post('infocred', TRUE);
            
            $antecedentes_resultado = $this->input->post('antecedentes_resultado', TRUE);
            
            $antecedentes_detalle = $this->input->post('antecedentes_detalle', TRUE);

            $nombre_usuario = $_SESSION["session_informacion"]["login"];
            $fecha_actual = date('Y-m-d H:i:s');

            if((int)$codigo_prospecto == 0 || $pep == "" || $match == "" || $infocred == "" || $antecedentes_resultado == -1 || $antecedentes_resultado > 2 || $antecedentes_detalle == "")
            {
            ...
            // Se procede a actualizar el prospecto para registrar la revisión de Antecedentes                        
                $this->mfunciones_logica->EvaluacionAntecedentesProspecto(1, $antecedentes_detalle, $pep, $match, $infocred, $nombre_usuario, $fecha_actual, $codigo_prospecto);
            
            // Dependiendo de la recomendación de CUMPLIMIENTO      1 = Aprueba     2 = Rechaza
            if($antecedentes_resultado == 1)
            {                
                ...
                $this->mfunciones_logica->InsertSeguimientoProspecto($codigo_prospecto, 1, 1, 'Completar Revisión Antecedentes', $nombre_usuario, $fecha_actual);
                
                /* Se Envía a la APP la Notificación por Push   tipo_notificacion, $tipo_visita, $codigo_visita */
                $this->mfunciones_generales->EnviarNotificacionPush(2, 1, $codigo_prospecto);                
            }
            
            if($antecedentes_resultado == 2)
            {
                // Se procede a actualizar el prospecto para registrar la revisión de Antecedentes                        
                ...
                /***  REGISTRAR SEGUIMIENTO ***/
                $this->mfunciones_logica->InsertSeguimientoProspecto($codigo_prospecto, 15, 7, 'Cumplimiento rechazó la Afiliación', $nombre_usuario, $fecha_actual);
