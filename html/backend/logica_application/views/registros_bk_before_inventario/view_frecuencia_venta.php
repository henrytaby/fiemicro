...
?>
    ...
    {
        var suma_total = 
        parseFloat($("#frec_mes_mes1_monto").val() || 0) + 
        parseFloat($("#frec_mes_mes2_monto").val() || 0) + 
        parseFloat($("#frec_mes_mes3_monto").val() || 0);

        $("#suma_meses").html("Bs. " + (suma_total/3).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,').replace('.', '|').replace(',', '.').replace('|', ','));
    }
