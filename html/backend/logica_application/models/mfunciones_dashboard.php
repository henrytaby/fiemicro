...
';
    ...
    // -- Tracking Oficiales de Negocio - INICIO

    function ArmarFiltro_tracking($sel_departamento, $sel_agencia, $sel_oficial, $campoFiltroFechaDesde, $campoFiltroFechaHasta, $estado)
    {
        $estado = (int)$estado;
        ...
        if($this->mfunciones_generales->VerificaFechaD_M_Y($campoFiltroFechaDesde))
        {
            $filtro .= " AND fecha_checkin >= '" . $this->mfunciones_generales->getFormatoFechaDateTime($campoFiltroFechaDesde . ' 00:00:01') . "'";
        }
        
        if($this->mfunciones_generales->VerificaFechaD_M_Y($campoFiltroFechaHasta))
        {
            $filtro .= " AND fecha_checkin <= '" . $this->mfunciones_generales->getFormatoFechaDateTime($campoFiltroFechaHasta . ' 23:59:59') . "'";
        }
    ...
    // -- Tracking Oficiales de Negocio - FIN
    ...
    function ObtenerDatosOficial_dashboard($parent_codigo=-1, $criterio='', $aux)
    {
        ...
        {   
            $sql = "SELECT er.estructura_regional_id, er.estructura_regional_nombre, e.ejecutivo_id, CONCAT_WS(' ', u.usuario_app, u.usuario_apm, u.usuario_nombres) as 'ejecutivo_nombre'
                    ...
                    WHERE u.usuario_activo=1 AND er.estructura_regional_estado=1 $filtro AND u.usuario_rol=2 AND er.estructura_regional_id IN (" . $criterio . ") ORDER BY u.usuario_app ASC";
            ...
            $sql = "SELECT tipo, codigo, agencia_id, agencia_departamento, agencia_ciudad, ejecutivo_id, etapa_id, desembolso_monto, desembolso_fecha, moneda, monto, fecha_registro
                    FROM(
                        ...
                        FROM(
                            ...
                            INNER JOIN usuarios u1 ON u1.usuario_id=e1.usuario_id AND u1.usuario_activo=1
                            INNER JOIN estructura_regional er1 ON er1.estructura_regional_id=p1.codigo_agencia_fie AND er1.estructura_regional_estado=1
                    ...
                    UNION ALL
                        ...
                        FROM(
                            ...
                            INNER JOIN usuarios u2 ON u2.usuario_id=e2.usuario_id AND u2.usuario_activo=1
    ...
    {        
        try 
        {
            $sql = "SELECT DISTINCT e.ejecutivo_id, CONCAT_WS(' ', u.usuario_nombres, u.usuario_app, u.usuario_apm) as 'ejecutivo_nombre'
                    FROM ejecutivo e
                    INNER JOIN usuarios u ON u.usuario_id=e.usuario_id AND u.usuario_activo=1
    ...
    function ObtenerDatosDepartamento_dashboard_generico($criterio, $aux_estado)
    {        
        try 
        {
            // Filtro Estado
            if($aux_estado != 2)
            {
                $aux_estado = 'estructura_regional_estado=' . $aux_estado . ' AND';
            }
            else
            {
                $aux_estado = '';
            ...
            $sql = "SELECT dep
                    FROM(
                        SELECT DISTINCT estructura_regional_departamento as 'dep'
                        FROM estructura_regional
                        WHERE $aux_estado length(estructura_regional_departamento) >= 2 AND estructura_regional_ciudad <> 115 AND estructura_regional_id IN(" . $criterio . ")
                        GROUP BY estructura_regional_departamento
                    ...
                    UNION ALL
                        ...
                        SELECT 'LPALTO' as 'dep'
                        FROM estructura_regional
                        WHERE $aux_estado estructura_regional_ciudad='115' AND estructura_regional_id IN(" . $criterio . ")
                    ) a GROUP BY dep ORDER BY dep ASC";
    ...
    function ObtenerDatosRegional_dashboard_generico($parent_codigo=-1, $parent_tipo=-1, $criterio='', $aux, $aux_estado)
    {        
        try 
        {
            $parent_codigo = htmlspecialchars($parent_codigo);
            ...
            if($aux_estado != 2)
            {
                $aux_estado = 'r.estructura_regional_estado=' . $aux_estado . ' AND';
            }
            else
            {
                $aux_estado = '';
            ...
            $sql = "SELECT DISTINCT r.estructura_regional_id, r.estructura_regional_estado, r.estructura_regional_nombre, r.estructura_regional_departamento, r.estructura_regional_ciudad
                        FROM estructura_regional r 
                        WHERE $aux_estado length(estructura_regional_nombre) >= 2 $filtro AND r.estructura_regional_id IN (" . $criterio . ") ORDER BY r.estructura_regional_nombre ASC";
    ...
    function ObtenerDatosOficial_dashboard_generico($parent_codigo=-1, $criterio='', $aux, $aux_estado)
    {
        $parent_codigo = htmlspecialchars($parent_codigo);
        $criterio = htmlspecialchars($criterio);
        ...
        {   
            $sql = "SELECT u.usuario_activo, er.estructura_regional_id, er.estructura_regional_nombre, e.ejecutivo_id, CONCAT_WS(' ', u.usuario_app, u.usuario_apm, u.usuario_nombres) as 'ejecutivo_nombre'
                    ...
                    INNER JOIN usuarios u ON u.estructura_agencia_id=ea.estructura_agencia_id
                    INNER JOIN ejecutivo e ON e.usuario_id=u.usuario_id
                    WHERE $aux_estado $filtro AND u.usuario_rol=2 AND er.estructura_regional_id IN (" . $criterio . ") ORDER BY u.usuario_app ASC";
    ...
    function ObtenerListaAgencia_generico($filtro)
    {        
        try 
        {
            $sql = "SELECT DISTINCT estructura_regional_id, estructura_regional_nombre, estructura_regional_estado
                    FROM estructura_regional
                    WHERE 1 $filtro"; 
    ...
    function ObtenerListaOficial_generico($filtro)
    {        
        try 
        {
