...
class Formulario_logica_general {
    ...
    public function DefinicionValidacionFormulario() {
        ...
        $this->formulario_campos->CargarOpcionesValidacion('general_solicitante', LETRAS_NUMEROS, 'MAX(190)|REQUERIDO|', '!|@|#|$|%|&|/|¿|?|¡|+|.|*|_|-|°|"|\'|(|)|,');
        ...
        $this->formulario_campos->CargarOpcionesValidacion('conf_sms_channelid', NUMEROS, 'MAX(9)|REQUERIDO|SINESPACIO|TIPO-NUMBER', '');
        $arr_validacion['conf_sms_channelid'] = clone $this->formulario_campos;
    ...
    public function ConstruccionCajasFormulario($arrValoresPorDefecto) {
        ...
        $id_caja = 'conf_sms_tiempo_validez';
        $strValorCaja = isset($arrValoresPorDefecto[$id_caja]) ? $arrValoresPorDefecto[$id_caja] : '';
        
        // Catalogo ;
        $arr_formulario_cajas[$id_caja] = html_select($id_caja, $combo_conf_f_cobis_intentos_tiempo, 'id', 'campoDescrip', 'SINSELECCIONAR', $strValorCaja);
        
            $combo_conf_sms_permitido_cantidad = array();
            for($intentos=1; $intentos<100; $intentos++) // the interval for hours is '1'
            {
                $combo_conf_sms_permitido_cantidad[] = array("id" => $intentos, "campoDescrip" => ($intentos==0 ? 'Sin límite' : $intentos));
            }
        
        $id_caja = 'conf_sms_permitido_cantidad';
        $strValorCaja = isset($arrValoresPorDefecto[$id_caja]) ? $arrValoresPorDefecto[$id_caja] : '';
        
        // Catalogo ;
        $arr_formulario_cajas[$id_caja] = html_select($id_caja, $combo_conf_sms_permitido_cantidad, 'id', 'campoDescrip', 'SINSELECCIONAR', $strValorCaja);
        
            $combo_conf_sms_permitido_tiempo = $combo_conf_f_cobis_intentos_tiempo;
            for($intentos=25; $intentos<=72; $intentos++) // the interval for hours is '1'
            {
                $combo_conf_sms_permitido_tiempo[] = array("id" => $intentos*60, "campoDescrip" => $intentos . ' horas');
            }
        
        $id_caja = 'conf_sms_permitido_tiempo';
        $strValorCaja = isset($arrValoresPorDefecto[$id_caja]) ? $arrValoresPorDefecto[$id_caja] : '';
        // Catalogo ;
        $arr_formulario_cajas[$id_caja] = html_select($id_caja, $combo_conf_sms_permitido_tiempo, 'id', 'campoDescrip', 'SINSELECCIONAR', $strValorCaja);
        
        $id_caja = 'conf_sms_onb_ambiente';
        $strValorCaja = isset($arrValoresPorDefecto[$id_caja]) ? $arrValoresPorDefecto[$id_caja] : '';
        $arr_frec_seleccion = Array(
                array("id" => "0", "campoDescrip" => "Si (Ambiente Producción)"),
                array("id" => "1", "campoDescrip" => "No (Ambiente Desarrollo)")
            );
        $arr_formulario_cajas[$id_caja] = html_select($id_caja, $arr_frec_seleccion, 'id', 'campoDescrip', 'SINSELECCIONAR', $strValorCaja);
        
        // Tracking Oficiales de Negocio
        
        $id_caja = "tipo_registro";
        $strValorCaja = isset($arrValoresPorDefecto[$id_caja]) ? $arrValoresPorDefecto[$id_caja] : "";
        $arrayTipoContratacion = Array(
            array("id" => "1", "campoDescrip" => "Estudio de Crédito"),
            array("id" => "2", "campoDescrip" => "Solicitud de Crédito"),
            array("id" => "99", "campoDescrip" => "Estudio y Solicitud de Crédito")
        );
        $arr_formulario_cajas[$id_caja] = html_select($id_caja, $arrayTipoContratacion, 'id', 'campoDescrip', 'SINSELECCIONAR', $strValorCaja);
        ...
        $strValorCaja = isset($arrValoresPorDefecto[$id_caja]) ? $arrValoresPorDefecto[$id_caja] : "";
        $arrayTipoContratacion = Array(
            array("id" => "2", "campoDescrip" => "Todos"),
            array("id" => "1", "campoDescrip" => "Abierta/Activo"),
            array("id" => "0", "campoDescrip" => "Cerrada/Inactivo")
        );
        $arr_formulario_cajas[$id_caja] = html_select($id_caja, $arrayTipoContratacion, 'id', 'campoDescrip', 'SINSELECCIONAR', $strValorCaja);
        
        $id_caja = "formato_reporte";
        $strValorCaja = isset($arrValoresPorDefecto[$id_caja]) ? $arrValoresPorDefecto[$id_caja] : "";
        $arrayTipoContratacion = Array(
            array("id" => "1", "campoDescrip" => "Tabla"),
            array("id" => "2", "campoDescrip" => "Tabla con Paginación"),
            array("id" => "3", "campoDescrip" => "Mapa"),
            array("id" => "4", "campoDescrip" => "Mapa agrupado (clusterer)")
        );
        $arr_formulario_cajas[$id_caja] = html_select($id_caja, $arrayTipoContratacion, 'id', 'campoDescrip', 'SINSELECCIONAR', $strValorCaja);
